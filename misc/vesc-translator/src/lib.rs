...
impl Getter for GetValues {
    ...
    fn parse_response<'a>(buffer: &'a Self::Input<'a>) -> Result<Self::Response, CorruptedResponse> {
        ...
        Ok(GetValuesResponse {
            ...
            motor_current: unscale_and_unpack([payload[4], payload[5], payload[6], payload[7]], 100.0),
            input_current: unscale_and_unpack([payload[8], payload[9], payload[10], payload[11]], 100.0),
...
fn extract_payload(buffer: &[u8], payload_len: usize) -> Result<&[u8], CorruptedResponse> {
    ...
    let received_checksum = u16::from_be_bytes([buffer[payload_len + header_size], buffer[payload_len + header_size + 1]]);
