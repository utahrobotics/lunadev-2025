...
impl<F: FnMut(f64) -> usize + Sync> Clusterer<F> {
    pub fn new(tolerance: f64, heatmap_scale: f64, heatmap_width: usize, min_points: usize, density: F) -> Self {
        Self { tolerance, heatmap_scale, heatmap_width, density, min_points, points_buffer: Vec::new(), sum_buffer: FxHashMap::default() }
    }
    ...
    pub fn cluster<'a>(&'a mut self, heatmap: &[f64]) -> impl Iterator<Item=Vector2<f64>> + 'a {
        ...
        for (i, id) in clusters.indexed_iter().filter_map(|(i, &id)| id.map(|id| (i, id))) {
        ...
        self.sum_buffer.drain().map(|(_, (sum, count))| sum / count as f64)
    }
}