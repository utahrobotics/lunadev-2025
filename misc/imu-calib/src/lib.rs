...
impl CalibrationParameters {
    ...
    pub fn correct_gyroscope(&self, raw: &Vector3<f64>) -> Vector3<f64> {
        ...
        Vector3::new(
            rotated.x / self.gyroscope_scale.x,
            rotated.y / self.gyroscope_scale.y,
            rotated.z / self.gyroscope_scale.z,
        )
    }
}
...
impl Calibrator {
    ...
    pub fn avg_accel_vector(&self) -> Option<Vector3<f64>> {
        ...
        Some(sum / (self.sample_count() as f64))
    }
    ...
    /// Jayden's method for estimating the initial misalignment based on the average acceleration vector.
    ...
    pub fn calibrate(&mut self, with_scaling: bool) -> Result<CalibrationParameters, String> {
        ...
        println!("Estimated Initial Misalignment (Quaternion): {:?}", initial_misalignment);
        println!("Estimated Initial Misalignment (Euler): {:?}", initial_misalignment.euler_angles());
...
impl<'a> Function for ImuCostFunction<'a> {
    fn value(&self, params_vec: &[f64]) -> f64 {
        ...
        for (accel_raw, gyro_raw) in self.readings {
            ...
            let accel_error = accel_diff.norm_squared().powi(2);
            let gyro_diff = gyro_corrected - ideal_gyro;
            let gyro_error = gyro_diff.norm_squared().powi(2);
