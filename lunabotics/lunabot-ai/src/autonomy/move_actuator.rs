...
use ares_bt::{converters::AssertCancelSafe, looping::WhileLoop, sequence::{ParallelAny, ParallelSequence, Sequence}, Behavior, CancelSafe, Status};
    ...
    Sequence::new((
        ...
        AssertCancelSafe(|blackboard: &mut LunabotBlackboard| {
            ...
            if *blackboard.get_tilt_travel_positive() {
                tilt_done = blackboard.get_actual_tilt() >= *blackboard.get_target_tilt() - COMPLETION_TOLERANCE;
                if tilt_done {
            ...
            if *blackboard.get_lift_travel_positive() {
                lift_done = blackboard.get_actual_lift() >= *blackboard.get_target_lift() - COMPLETION_TOLERANCE;
                if lift_done {
            ...
            if lift_done && tilt_done {
                ...
                *blackboard.get_poll_when() = PollWhen::Instant(blackboard.get_now() + Duration::from_millis(50));
                Status::Running
            }
        }),
    ))
