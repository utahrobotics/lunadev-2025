...
fn follow_path_inner(blackboard: &mut LunabotBlackboard) -> Status {
    ...
    match latest_transform {
        ...
        Some((prev_pos, prev_rot, time_of_prev_pos)) => {
            ...
            if distance(&prev_pos, &pos) > MIN_DIST_UNTIL_TRANSFORM_UPDATE
...
fn do_then_wait(
    behavior: impl Behavior<LunabotBlackboard> + CancelSafe,
    duration: Duration,
) -> impl Behavior<LunabotBlackboard> + CancelSafe {
    TryCatch::new(
        Sequence::new((behavior, WaitBehavior::from(duration))),
