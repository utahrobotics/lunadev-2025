...
pub struct IMUReadingTimestamped(Vector3<f32>, Vector3<f32>, TIMESTAMP);
...
pub fn enumerate_imus(
    ...
    let data_queue: Box<ArrayQueue<IMUReadingTimestamped>> = Box::new(ArrayQueue::new(64));
    let data_queue_ref: &'static ArrayQueue<IMUReadingTimestamped> = Box::leak(data_queue);
    ...
    std::thread::spawn(move || {
        ...
        let mut reading: Option<IMUReadingTimestamped> = None;
        loop {
            reading = data_queue_ref.pop();
            if let Some(IMUReadingTimestamped(gyro, accel, time)) = reading {
...
struct IMUTask {
    ...
    localizer_ref: LocalizerRef,
    index: usize,
    queue: &'static ArrayQueue<IMUReadingTimestamped>,
}
        ...
        loop {
            ...
            match msg {
                FromIMU::Reading(rate, accel) => {
                    ...
                    if let Err(_) = self.queue.push(IMUReadingTimestamped(
                        transformed_rate,
                        transformed_accel,
