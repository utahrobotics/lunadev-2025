...
pub enum FromIMU {
    ...
    AccellerationNormReading(AccellerationNorm),
}
...
#[derive(Clone, Copy, Debug, Format)]
pub struct AccellerationNorm {
    pub x: f32,
    pub y: f32,
    pub z: f32
}

impl AccellerationNorm {
    ...
    /// Deserializes from bytes into an AccellerationNorm struct
...
mod tests {
    ...
    fn test_acceleration_serialization_roundtrip() {
        let original = AccellerationNorm {
            x: 1.0,
            y: -2.5,
            z: 0.5,
        };
        ...
        let deserialized = AccellerationNorm::deserialize(serialized).unwrap();
